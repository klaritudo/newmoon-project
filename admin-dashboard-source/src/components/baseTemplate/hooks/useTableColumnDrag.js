 ...
 * @param {string} options.version - 컬럼 정의 버전 (버전이 다르면 캐시 무효화)
...
const useTableColumnDrag = ({
  ...
  useEffect(() => {
    ...
    if (savedVersion !== version) {
      // 버전이 다르면 해당 테이블의 모든 설정 초기화
      console.log(`테이블 ${tableId}의 버전이 변경되었습니다. 캐시를 초기화합니다.`);
  ...
  const handleDrop = useCallback((event, targetColumnId, isTargetGroupColumn = false, targetParentGroupId = null) => {
    ...
    if (enableColumnPinning) {
      ...
      // 타겟이 그룹 헤더인 경우 특별 처리
      if (targetColumnIndex === -1 && isTargetGroupColumn) {
        // 그룹 헤더의 첫 번째 자식 컬럼을 기준으로 위치 판단
        const targetGroup = columns.find(col => col.id === targetColumnId);
        if (targetGroup && targetGroup.children && targetGroup.children.length > 0) {
          const firstChildId = targetGroup.children[0].id;
          targetColumnIndex = allRenderColumns.indexOf(firstChildId);
        }
      }
      
      // 드롭 위치가 고정영역인지 비고정영역인지 판단
      // targetColumnIndex가 -1인 경우(찾을 수 없는 경우)는 비고정영역으로 간주
      isDropInPinnedArea = targetColumnIndex !== -1 && targetColumnIndex <= lastPinnedColumnIndex;
