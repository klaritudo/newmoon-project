 ...
 * @param {Object} props.summary - 합계 기능 설정
 * @param {Function} props.onSummaryToggle - 합계 토글 이벤트 핸들러
 * @param {boolean} props.fixedFooter - 테이블 푸터(합계 행) 고정 사용 여부
 */
const BaseTable = ({
  columns = [],
  data = [],
  ...
  useEffect(() => {
    console.log(`[BaseTable] 상태:`, {
      ...
      firstRowData: data?.[0],
      columns: columns.map(col => ({ id: col.id, type: col.type, label: col.label }))
    });
  }, [page, rowsPerPage, totalCount, data, sequentialPageNumbers, columns]);
  ...
  // 합계 계산 함수
  const calculateSummaryData = useCallback((targetData, summaryConfig) => {
    if (!summaryConfig?.columns || !targetData?.length) return {};
    
    const result = {};
    
    Object.entries(summaryConfig.columns).forEach(([columnId, config]) => {
      if (!config) return;
      
      // 조건부 필터링
      let filteredData = targetData;
      if (config.condition) {
        filteredData = targetData.filter(row => config.condition(row));
      }
      
      
      const values = filteredData
        .map(row => {
          // 중첩된 필드 처리 (예: 'deposit_withdrawal.charge')
      ...
      switch (config.type) {
        case 'sum':
          ...
          break;
        case 'avg':
          result[columnId] = values.length > 0 
            ? values.reduce((sum, val) => sum + Number(val), 0) / values.length 
            : 0;
          break;
        case 'count':
          result[columnId] = filteredData.length;
          break;
        case 'min':
          result[columnId] = values.length > 0 ? Math.min(...values.map(Number)) : 0;
          break;
        case 'max':
          result[columnId] = values.length > 0 ? Math.max(...values.map(Number)) : 0;
          break;
        case 'custom':
          if (config.customCalculator) {
            result[columnId] = config.customCalculator(values, filteredData);
          }
          break;
        default:
          result[columnId] = 0;
  ...
  const currentPageData = useMemo(() => {
    ...
    const flattenData = (items) => {
      ...
      items.forEach(item => {
        result.push(item);
        if (item.children && item.children.length > 0 && expandedRows[item.id]) {
          result = [...result, ...flattenData(item.children)];
        }
      });
      return result;
    ...
    const flatData = hierarchical ? flattenData(data) : data;
    const startIndex = page * rowsPerPage;
    const endIndex = startIndex + rowsPerPage;
    
    return flatData.slice(startIndex, endIndex);
  }, [data, page, rowsPerPage, hierarchical, expandedRows]);
  
  // 필터링된 데이터 계산
  const filteredData = useMemo(() => {
    if (!summary?.scope?.customFilter) return data;
    
    const filterData = (items) => {
      return items.filter(item => {
        const passFilter = summary.scope.customFilter(item);
        if (passFilter && item.children && item.children.length > 0) {
  ...
  const summaryData = useMemo(() => {
    if (!summary?.enabled || !data?.length) {
      return null;
    }
    ...
    // 범위에 따른 데이터 선택
    if (summary.scope?.type === 'page' || showCurrentPageOnly) {
      targetData = currentPageData;
      
      // 현재 페이지 데이터에도 customFilter 적용
      if (summary.scope?.customFilter) {
        targetData = targetData.filter(row => summary.scope.customFilter(row));
      }
    } else if (summary.scope?.type === 'filtered') {
      targetData = filteredData;
    } else if (summary.scope?.type === 'all' && summary.scope?.customFilter) {
      // 'all' 타입이지만 customFilter가 있으면 적용
      
      // 계층 구조 데이터를 평면화한 후 필터 적용
      if (hierarchical) {
        const flattenHierarchicalData = (items, level = 0) => {
          let result = [];
          items.forEach(item => {
            result.push(item);
            if (item.children && item.children.length > 0) {
              result = [...result, ...flattenHierarchicalData(item.children, level + 1)];
            }
          });
          return result;
        };
        ...
        targetData = flatData.filter(row => summary.scope.customFilter(row));
      } else {
        targetData = data.filter(row => summary.scope.customFilter(row));
      }
    }
    
    const result = calculateSummaryData(targetData, summary);
    return result;
  }, [data, summary, showCurrentPageOnly, currentPageData, filteredData, calculateSummaryData, hierarchical]);
  
  // 합계 토글 핸들러
  const handleSummaryToggle = useCallback((currentPageOnly) => {
  ...
  //     dragHandlersKeys: Object.keys(dragHandlers || {})
  //   });
  // }, [columns, data, checkable, pinnedColumns, draggableColumns, dragHandlers]);
  ...
  return (
    ...
    >
      ...
      >
        {/* 상단 합계 */}
        {summary?.enabled && (summary?.position === 'top' || summary?.position === 'both') && (
          <TableHead>
            <TableSummary
              columns={columns}
        ...
        />
        {/* 하단 합계 */}
        {summary?.enabled && (summary?.position === 'bottom' || summary?.position === 'both') && (
          <TableFooter>
            <TableSummary
              columns={columns}
              summaryData={summaryData}
...
BaseTable.propTypes = {
  ...
  summary: PropTypes.shape({
    ...
    scope: PropTypes.shape({
      type: PropTypes.oneOf(['page', 'filtered', 'all']),
  ...
  onSummaryToggle: PropTypes.func,
