...
// 상태별 칩 스타일 정의
export const getWithdrawalStatusChipStyle = (status) => {
  switch (status) {
    case 'pending':
...
// 출금문의 컬럼 정의
export const withdrawalInquiriesColumns = [
  {
    id: 'no',
    label: 'No.',
    width: 80,
  ...
  },
  {
    ...
    sortable: false,
    type: 'chip',
    render: () => ({ label: '출금', color: 'secondary', variant: 'outlined' })
  },
  {
    id: 'username',
    label: '아이디(닉네임)',
  ...
  },
  {
    ...
    sortable: true,
    type: 'custom',
    render: (row) => `${Math.floor(row.amount || 0).toLocaleString()}원`
  },
  {
    id: 'before_balance',
    label: '처리전금액',
    ...
    render: (row) => {
      ...
      if (row.status === 'pending' || row.status === 'waiting') {
        ...
        // 출금 승인된 경우: 현재잔액 + 출금액 = 처리전금액
        const beforeBalance = (row.current_balance || 0) + (row.amount || 0);
  ...
  },
  {
    ...
    render: (row) => (
      <Chip
        ...
        size="small"
        {...getWithdrawalStatusChipStyle(row.status)}
      />
    )
  },
  {
...
// 컬럼 버전 관리 - 입금신청처리와 동일한 버전 사용
// v1.0.2: actions 컬럼 type을 'default'로 변경
// v1.0.3: 액션 버튼을 IconButton에서 Button으로 변경 (입금신청처리와 일관성)
// v1.0.4: No. 컬럼을 id에서 no로 변경하여 순차 번호 표시
// v1.0.5: type: 'default' 제거 및 render 함수 문제 해결
// v1.0.6: 모든 render 함수가 있는 컬럼에 type: 'custom' 추가
// v1.0.7: processor_username 컬럼 제거 (DB에 processor_id 필드 없음)
// v1.0.8: 모든 render 함수에 type: 'custom' 추가 및 processor 컬럼 추가
// v1.0.9: 처리전금액/처리후금액 컬럼 추가, 금액->신청금액 변경, 소수점 제거
// v1.0.10: 처리자 컬럼에 실제 처리한 관리자 아이디 표시
export const WITHDRAWAL_INQUIRIES_COLUMNS_VERSION = '1.0.10';