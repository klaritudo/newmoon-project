...
import { 
  withdrawalInquiriesColumns,
  withdrawalStatusOptions,
  getWithdrawalStatusChipStyle,
  getStatusText,
  WITHDRAWAL_INQUIRIES_COLUMNS_VERSION
} from './data/withdrawalInquiriesData.jsx';
...
import withdrawalInquiriesService from '../../services/withdrawalInquiriesService';
...
const WithdrawalPage = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // 상태별 카운트 조회
      const countsResponse = await withdrawalInquiriesService.getCounts();
      ...
      // 데이터 조회
      const response = await withdrawalInquiriesService.getAll(params);
      ...
      console.error('출금문의 데이터 로드 실패:', err);
  ...
  // 컬럼 버전 체크 및 초기화
  useEffect(() => {
    const savedVersion = localStorage.getItem('withdrawalPage_columnsVersion');
    const currentVersion = WITHDRAWAL_INQUIRIES_COLUMNS_VERSION.toString();
    
    if (savedVersion !== currentVersion) {
      // 버전이 다르면 저장된 컬럼 설정 초기화
      localStorage.removeItem('withdrawalPage_columnVisibility');
      localStorage.removeItem('withdrawalPage_columnOrder');
      localStorage.removeItem('withdrawalPage_pinnedColumns');
      localStorage.setItem('withdrawalPage_columnsVersion', currentVersion);
  ...
  useEffect(() => {
    ...
    // 출금문의 생성 이벤트
    const handleWithdrawalInquiryCreated = (data) => {
      console.log('새 출금문의:', data);
    ...
    // 출금문의 상태 변경 이벤트
    const handleWithdrawalInquiryStatusChanged = (data) => {
      console.log('출금문의 상태 변경:', data);
    ...
    // withdrawal:inquiry:created 이벤트는 유저 API에서 발생
    // withdrawal:inquiry:new 이벤트는 관리자 API의 내부 API를 통해 발생
    socketService.on('withdrawal:inquiry:created', handleWithdrawalInquiryCreated);
    socketService.on('withdrawal:inquiry:new', handleWithdrawalInquiryCreated);
    socketService.on('withdrawal:inquiry:status:changed', handleWithdrawalInquiryStatusChanged);

    return () => {
      socketService.off('withdrawal:inquiry:created', handleWithdrawalInquiryCreated);
      socketService.off('withdrawal:inquiry:new', handleWithdrawalInquiryCreated);
      socketService.off('withdrawal:inquiry:status:changed', handleWithdrawalInquiryStatusChanged);
    };
  ...
  const handleStatusChange = useCallback(async (row, newStatus, rejectReason = '') => {
    try {
      const response = await withdrawalInquiriesService.updateStatus(row.id, {
        status: newStatus,
        rejectReason
      });
      
      if (response.success) {
        alert(`출금문의가 ${getStatusText(newStatus)} 처리되었습니다.`);
        loadData();
      }
    } catch (err) {
  ...
  const columnsWithActions = useMemo(() => {
    return withdrawalInquiriesColumns.map(column => {
  ...
  const {
    ...
    showAllColumns,
    resetToDefault
  } = useColumnVisibility(withdrawalInquiriesColumns, {
        ...
        {tableCheckedItems.length > 0 && activeTab === 'pending' && (
          <Box sx={{ mb: 2, display: 'flex', gap: 1 }}>
            <Button
              ...
              size="small"
              onClick={async () => {
                if (confirm(`선택한 ${tableCheckedItems.length}건을 승인하시겚습니까?`)) {
                  try {
                    const response = await withdrawalInquiriesService.bulkUpdate({
            ...
            <Button
              ...
              size="small"
              onClick={async () => {
                if (confirm(`선택한 ${tableCheckedItems.length}건을 대기 처리하시겚습니까?`)) {
                  try {
                    const response = await withdrawalInquiriesService.bulkUpdate({
                      ids: tableCheckedItems,
                      status: 'waiting'
            ...
            <Button
              ...
              onClick={async () => {
                const reason = prompt('비승인 사유를 입력하세요:');
                if (reason !== null) {
                  if (confirm(`선택한 ${tableCheckedItems.length}건을 비승인하시겚습니까?`)) {
                    try {
                      const response = await withdrawalInquiriesService.bulkUpdate({
                        ids: tableCheckedItems,
                        status: 'rejected',
